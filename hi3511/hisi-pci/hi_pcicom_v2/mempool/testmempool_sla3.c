...
static int __init testmempool_sla_init(void)
{
	...
	*/
	handle = hios_mempool_create("mempool_slave3", 10*1024,0,127,0);
	...
	*/
	...
	/*填充内存块信息*/
	msleep(500);
	memblk[0] = 0xA0000000;      //内存块首地址
	memblk[1] = memblk[0] + 0x100000; //内存块尾地址
	memblk[2] = 0xA2000000;           //内存块首地址
	...
	*/
	...
	mmb[0].handle=0x200001;
	mmb[0].addr = 0xA2032000;
	mmb[0].size=0x32000;
	hios_mempool_free(mmb[0]);	
	
	mmb[0].handle=0x200001;
	mmb[0].addr = 0xA2096000;
	mmb[0].size=0x32000;
	hios_mempool_free(mmb[0]);
