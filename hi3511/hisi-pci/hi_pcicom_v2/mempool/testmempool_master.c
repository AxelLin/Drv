...
static int __init testmempool_init(void)
{
	hios_u32 mempool[10];
	hios_s32 i,count=0;
	hios_u32 handle ,handle1,handle2;
	...
	*/	
	...
	handle1 = hios_mempool_create("mempool_host1", 100*1024,2,127,0);
	printk("create sucess! handle1 = %x\n",handle1);
	handle2 = hios_mempool_create("mempool_host2", 100*1024,3,126,0);
	printk("create sucess! handle2 = %x\n",handle2);
  ...
  msleep(3000);
	...
	*/
	while(1)
	{
		...
		addr = hios_mempool_alloc(&mmb[count+10], handle1, 0x20000);
		if(addr)
		{
			printk("alloc sucess! addr = 0x%x\n",(unsigned int)addr);
			printk("mmb.handle=0x%x\n",mmb[count+10].handle);
		...
		addr = hios_mempool_alloc(&mmb[count+20], handle2, 0x20000);
		if(addr)
		{
			printk("alloc sucess! addr = 0x%x\n",(unsigned int)addr);
			printk("mmb.handle=0x%x\n",mmb[count+20].handle);
	...
	addr = hios_mempool_alloc(&mmb[count], handle1, 0x32000);
	if(addr)
	{
		printk("alloc sucess! addr = 0x%x\n",(unsigned int)addr);
		printk("mmb.handle=0x%x\n",mmb[count].handle);
	...
	addr = hios_mempool_alloc(&mmb[count], handle1, 0x32000*2);
	if(addr)
	{
		printk("alloc sucess! addr = 0x%x\n",(unsigned int)addr);
		printk("mmb.handle=0x%x\n",mmb[count].handle);
	...
	addr = hios_mempool_alloc(&mmb[count], handle2, 0x32000);
	if(addr)
	{
		printk("alloc sucess! addr = 0x%x\n",(unsigned int)addr);
		printk("mmb.handle=0x%x\n",mmb[count].handle);
	...
	addr = hios_mempool_alloc(&mmb[count], handle2, 0x32000*2);
	if(addr)
	{
		printk("alloc sucess! addr = 0x%x\n",(unsigned int)addr);
		printk("mmb.handle=0x%x\n",mmb[count].handle);
