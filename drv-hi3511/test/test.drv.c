...
static int __init xr20m1172_gpio_init(void)
{
	...
	i2c_gpio_byte_write(XR20M1172_ADDR, reg_info[XR20M1170REG_LCR][0], 0x55);
	...
	i2c_gpio_byte_write(XR20M1172_ADDR, 0x05, 0x66);
	printk("i2c_gpio write 0x66 \n");




	 temp = i2c_gpio_byte_read(XR20M1172_ADDR, 0x05);
	...
	err =  i2c_gpio_byte_write(XR20M1172_ADDR, reg_info[XR20M1170REG_IOSTATE][0], temp);
	
	if(err)
		{
			printk("i2c gpio write  failed !");
