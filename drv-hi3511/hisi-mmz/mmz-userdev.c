...
static int mmz_userdev_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if(_IOC_TYPE(cmd) == 'm') {
		...
		copy_from_user(&mi, (void*)arg, _IOC_SIZE(cmd));

		ret = mmz_userdev_ioctl_m(inode, file, cmd, &mi);

		if(!ret && (cmd&IOC_OUT))
			copy_to_user((void*)arg, &mi, _IOC_SIZE(cmd));
...
static struct himedia_device mmz_userdev = {
...
{
	himedia_deregister(&mmz_userdev);
}
