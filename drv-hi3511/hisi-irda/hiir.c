...
static unsigned int HIIR_DEVICE_MAJOR = 0;
/* irq */
#define HIIR_DEVICE_IRQ_NO 9

/* define IR IO */
#define IR_REG_BASE (IO_ADDRESS(0x101e9000))
...
/*外设控制寄存器51 14:13 bit: SPI与IR以?GPIO复用控制:10：IR输入*/
#define SCPERCTRL1  (IO_ADDRESS(0x101E0020))
...
static void hiir_config(void)
{
    ...
    value = READ_REG(SCPERCTRL1);
    value &= ~(1<<16);
    WRITE_REG(SCPERCTRL1, value);
...
static irqreturn_t hiir_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
...
static int hiir_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case IR_IOC_GET_CONFIG:
            copy_to_user((void __user *)arg, &(hiir_dev.dev_parm), sizeof(hiir_dev_param));
...
retry :
    ...
    if((hiir_dev.head)==(hiir_dev.tail))
    {
        if((filp->f_flags & O_NONBLOCK) == O_NONBLOCK)
            return -EAGAIN;
        interruptible_sleep_on(&(hiir_dev.irkey_wait));
    ...
    else
    {
        while( ((hiir_dev.head)!=(hiir_dev.tail)) && ((res + sizeof(irkey_info_s)) <= count) )
        {
            ...
            copy_to_user((buf + res), &irkey_to_user, sizeof(irkey_info_s));
...
static int __init hiir_init(void)
{
    ...
    res = register_chrdev(HIIR_DEVICE_MAJOR, HIIR_DEVICE_NAME, &hiir_fops);
    if(res < 0)
    {
    ...
    if(0 == HIIR_DEVICE_MAJOR)
    {
        HIIR_DEVICE_MAJOR = res;
    }
    hiir_dbg("DEVICE_MAJOR = %d.\n", HIIR_DEVICE_MAJOR);

    res = request_irq(HIIR_DEVICE_IRQ_NO, &hiir_interrupt, SA_SHIRQ|SA_INTERRUPT, HIIR_DEVICE_NAME, &hiir_interrupt);
    if(res)
    {
        ...
        devfs_mk_cdev(MKDEV(HIIR_DEVICE_MAJOR, 0), S_IFCHR|S_IRUGO|S_IWUSR, HIIR_DEVICE_NAME);
    #endif
...
static void __exit hiir_exit(void)
{
    ...
    #ifdef CONFIG_DEVFS_FS
        devfs_remove(HIIR_DEVICE_NAME);
    #endif
    unregister_chrdev(HIIR_DEVICE_MAJOR, HIIR_DEVICE_NAME);
}
