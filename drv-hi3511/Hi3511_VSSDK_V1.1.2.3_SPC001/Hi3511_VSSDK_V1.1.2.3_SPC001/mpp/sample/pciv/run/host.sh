#!/bin/sh
ROOT_DIR=$PWD

# If you want to run the pciv sample, you should set mmz as follow
rmmod mmz
modprobe mmz mmz=anonymous,0,0xE5000000,96M:window,0,0xE4100000,15M anony=1
#modprobe mmz mmz=anonymous,0,0xE4000000,96M

# Boot all the slave chip
cd $ROOT_DIR/boot/
./boot.sh

# Insmod all the pci drivers
cd $ROOT_DIR/pci_driver
insmod pci_hwhal_host.ko
insmod hi_hw_adp.ko
insmod pcicom.ko
insmod mcc_usrdev.ko
insmod hi3511_pci_trans.ko

# Load all the media drivers
cd $ROOT_DIR/meadia_driver/
./load3511

cd $ROOT_DIR/test
#./runhost.sh
