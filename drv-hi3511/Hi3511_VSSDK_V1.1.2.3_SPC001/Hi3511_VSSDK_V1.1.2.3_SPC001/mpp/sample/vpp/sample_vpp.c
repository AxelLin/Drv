 ...
 ******************************************************************************
  ...
  Function List :
              main
              SampleCtrl1PublicOverlayRegion
              SampleCtrl4OverlayRegion
              SampleCtrlCoverRegion8Cif
...
{	
	...
	{
		printf("HI_MPI_SYS_Exit err 0x%x\n",s32Ret);
	}
									...
									BITMAP_S*pumBitMap)
{
	...
	if(PIXEL_FORMAT_YUV_SEMIPLANAR_422 == enPixelFormat)
	{
		...
		pumBitMap->enPixelFormat = PIXEL_FORMAT_YUV_SEMIPLANAR_422;
...
HI_VOID *SampleCtrl1PublicOverlayRegion(HI_VOID *p)
{
	...
	while(1)
	{
		...
		else if(s32Cnt <= 30)
		{
			...
			unParam.stPoint.s32X = 100 + (s32Cnt - 20)*4;
			unParam.stPoint.s32Y = 100 + (s32Cnt - 20)*4;
...
HI_S32 SampleCtrlMosaicRegion8Cif(HI_VOID)
{
	...
	while(1)
	{
		...
		else if(s32Cnt <= 30)
		{
			/*change position*/
			enCtrl = REGION_SET_POSTION;
			unParam.stPoint.s32X = 50 + ((s32Cnt - 20) * 10);
...
HI_S32 SAMPLE_8Cif_1PublicCoverRegion()
{
	...
	if(HI_SUCCESS != SampleEnableViVo8Cif())
	{
		Sample2815Close();
		SampleSysExit();
...
HI_S32 SAMPLE_1D1_4OverlayRegion(HI_VOID)
{
	...
	pthread_create(&VencPid, 0, SampleGetStream, NULL);
...
{	
	...
	char ping[1024] =   "\n/**************************************/\n"
						...
						"5  --> SAMPLE_1D1_4MosaicRegion\n"
						"6  --> SAMPLE_8Cif_1PublicMosaicRegion\n"
						"q  --> quit\n"
						"please enter order:\n";
	...
	while((ch = getchar())!= 'q')
	{
		...
		switch(ch)
		{
			...
			case '5':
			{
				SAMPLE_1D1_4MosaicRegion();
				break;
			}
			
			case '6':
			{
				SAMPLE_8Cif_1PublicMosaicRegion(); 
				break;
			}
