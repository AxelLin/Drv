...
//modified by wzh for multicast filter 2009-3-24
static unsigned short multi_flags = 0;
static void hieth_net_set_multicast_list(struct net_device *dev)
{
	...
	if(dev->mc_count > 0)
	{
		...
		{
			if ((dev->mc_count > 2 || !dev->mc_list) && (0 != multi_flags) )
			{
           			printk(KERN_ERR "hw do not support more than two group-cast\n");
            			return ;
        		}

			hieth_set_mc_mac_filter(ld, dev->mc_list, dev->mc_count, dev->flags, multi_flags);
		}
		else
		{
			hieth_clear_mac_filter(ld);
		}
		multi_flags = (dev->flags & IFF_MULTICAST);
	}

	if(dev->mc_count < 1)
	{
		hieth_set_mc_mac_filter(ld, NULL, dev->mc_count, dev->flags, multi_flags);
	}
