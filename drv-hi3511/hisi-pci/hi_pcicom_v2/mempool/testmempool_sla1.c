...
static int __init testmempool_sla_init(void)
{
  ...
  hios_u32 port;
	...
	port = hios_mcc_getlocalid();
	/*使用前必须先初始化。输入参数为local_id，用于标识不同的CPU*/
	hios_mempool_init(port);
	...
	*/
	handle = hios_mempool_create("mempool_slave1", 10*1024,0,port+100,0);
	...
	*/
	...
	/*填充内存块信息*/
	msleep(500);
	memblk[0] = 0xC0000000 - (port-1)*0x10000000;      //内存块首地址
	memblk[1] = memblk[0] + 0x100000; //内存块尾地址
	memblk[2] = memblk[0] + 0x2000000;           //内存块首地址
	...
	*/
	...
	i = hios_add_mempool(handle, memblk, 2);
	if(i)//返回值非零，表示出错
		printk("ERROR! i= %d\n",i);	
	...
	*/
	...
	mmb[0].handle=handle;
	mmb[0].addr = 0xC0000000 - (port-1)*0x10000000 + 0x2032000;
	mmb[0].size=0x32000;
	hios_mempool_free(mmb[0]);	
	
	mmb[0].handle=handle;
	mmb[0].addr = 0xC0000000 - (port-1)*0x10000000 + 0x2096000;
	mmb[0].size=0x32000;
	hios_mempool_free(mmb[0]);
